# -*- coding: utf-8 -*-
"""
Created on Mon Sep 25 22:44:30 2017
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(300):
            all_sim[j] += (float(patient.update()))
    for i in range(300):
        all_sim[i] /= numTrials

    pylab.plot(range(300), all_sim)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if (random.random() < (self.getMaxBirthProb()*(1-popDensity))):
            resist_copy = self.resistances.copy()
            for drug in self.resistances:
                if random.random() < self.mutProb :
                    resist_copy[drug] = not self.resistances[drug]
            return ResistantVirus(self.getMaxBirthProb(), self.getClearProb(), resist_copy, self.mutProb)
        else:
            raise NoChildException  
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in self.viruses[:]:
            if virus.doesClear():
                self.viruses.remove(virus)
        vir_density = self.getTotalPop()/float(self.getMaxPop())
        for virus in self.viruses[:]:
            try:
                self.viruses.append(virus.reproduce(vir_density, self.drugs))
            except NoChildException:
                pass
